...
const FindRide = () => {
  ...
  const fetchAllRides = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/ride/all`, {
        headers: {
      ...
      Alert.alert(
        "Error",
        "Failed to fetch available rides. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const geocodeLocation = async (locationName) => {
    try {
      ...
      const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
        locationName
      )}&region=lk&key=${GOOGLE_MAPS_API_KEY}`;
      ...
      const fallbackCoordinates = {
        ...
        galle: {
          latitude: 6.0328,
          longitude: 80.217,
          display_name: "Galle, Sri Lanka",
        },
        jaffna: {
          latitude: 9.6613,
          longitude: 80.0255,
          display_name: "Jaffna, Sri Lanka",
        },
        gampaha: {
          latitude: 7.0917,
          longitude: 80.0,
          display_name: "Gampaha, Sri Lanka",
        },
      };
  ...
  const filterRides = async () => {
    ...
    try {
      ...
      const lngDelta =
        Math.abs(startCoord.longitude - endCoord.longitude) * 1.5;
  ...
  const showRideOnMap = (ride) => {
    ...
    try {
      ...
      const startCoord =
        typeof ride.startCoordinates === "string"
          ? JSON.parse(ride.startCoordinates)
          : ride.startCoordinates;

      const endCoord =
        typeof ride.endCoordinates === "string"
            ...
            <MapView style={styles.map} region={mapRegion}>
              ...
              {selectedRide && selectedRide.startCoordinates && (
                <Marker
                  coordinate={{
                    latitude:
                      typeof selectedRide.startCoordinates === "string"
                        ? JSON.parse(selectedRide.startCoordinates)[0]
                        : selectedRide.startCoordinates.latitude ||
                          selectedRide.startCoordinates[0],
                    longitude:
                      typeof selectedRide.startCoordinates === "string"
                        ? JSON.parse(selectedRide.startCoordinates)[1]
                        : selectedRide.startCoordinates.longitude ||
                          selectedRide.startCoordinates[1],
                  }}
                  title="Ride Start"
                  pinColor="blue"
                />
              ...
              {selectedRide && selectedRide.endCoordinates && (
                <Marker
                  coordinate={{
                    latitude:
                      typeof selectedRide.endCoordinates === "string"
                        ? JSON.parse(selectedRide.endCoordinates)[0]
                        : selectedRide.endCoordinates.latitude ||
                          selectedRide.endCoordinates[0],
                    longitude:
                      typeof selectedRide.endCoordinates === "string"
                        ? JSON.parse(selectedRide.endCoordinates)[1]
                        : selectedRide.endCoordinates.longitude ||
                          selectedRide.endCoordinates[1],
                  }}
                  title="Ride End"
                  pinColor="purple"
                />
            ...
            {loading ? (
              ...
              <View style={styles.initialStateContainer}>
                <Text style={styles.initialStateText}>
                  Enter your starting point and destination to find rides
                </Text>
              </View>
            )}
          </View>
